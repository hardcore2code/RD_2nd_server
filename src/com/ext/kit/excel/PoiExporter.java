...
public class PoiExporter {
    ...
    public Workbook export() {
        ...
        for (int i = 0; i < data.length; i++) {
            ...
            if (headers[i].length > 0) {
                ...
                for (int h = 0, lenH = headers[i].length; h < lenH; h++) {
                    if (cellWidth > 0) {
                        if(h == 3)
                        {
                        	sheet.setColumnWidth(h, cellWidth*4);
                        }
                        else if(h == 4 || h == 5)
                        {
                        	sheet.setColumnWidth(h, cellWidth*2);
                        }
                        else
                        {
            ...
            for (int j = 0, len = data[i] == null ? 0 : data[i].size(); j < len; j++) {
                ...
                if (obj instanceof Map) {
                    processAsMap(columns[i], row, obj,style);
                } else if (obj instanceof Model) {
                    processAsModel(columns[i], row, obj,style);
                } else if (obj instanceof Record) {
                    processAsRecord(columns[i], row, obj,style);
                } else {
    ...
    private static void processAsMap(String[] columns, Row row, Object obj,CellStyle style) {
        Cell cell;
    ...
    private static void processAsModel(String[] columns, Row row, Object obj,CellStyle style) {
        Cell cell;
    ...
    private static void processAsRecord(String[] columns, Row row, Object obj,CellStyle style) {
        Cell cell;
